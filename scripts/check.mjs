...
const roots=['api','media/hires'];
const files=['backend-init.js','catalog-data.js','shell.js','media-manifest.js'];
...
let failed=false;
for(const file of [...new Set(files)].sort()){const r=spawnSync(process.execPath,['--check',file],{stdio:'inherit'});if(r.status!==0)failed=true}
