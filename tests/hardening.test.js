...
test('owner operations revalidate the live staff record and covering indexes are durable',()=>{const edge=read('supabase/functions/maxines-api/index.ts'),sql=read('supabase/migrations/20260811_000002_performance_covering_indexes.sql');assert.match(edge,/const currentRole=await staffRole\(auth\.user\.id\)/);assert.match(edge,/includes\(currentRole\|\|''\)/);for(const idx of ['order_items_order_id_idx','order_items_product_id_idx','order_items_variant_id_idx','user_favorites_sku_idx'])assert.match(sql,new RegExp(idx))});
